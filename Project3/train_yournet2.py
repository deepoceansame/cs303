...
def train_student(T_model, S_model, train_loader, test_loader, device):
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            y = y.to(device)
            s_logits = S_model(x)
            loss = get_loss(t_train_logits[i], s_logits, y, 1, 0.1)
        ...
        torch.save(S_model.state_dict(), './checkpoints/YourNet2/' + f'epoch-{epoch}.pth')
...
if __name__ == '__main__':
    ...
    batch_size = 64
    device = 'cuda'
    checkpoint_dir = './checkpoints/YourNet2/'
